#' @title get_stp_server_config
#'
#' @description Gets the Stored Process Server information and its connections from the metadata server by a server name.
#'
...
#' @importFrom httr GET
#'
#' @export get_stp_server_config
get_stp_server_config <- function(url, repositoryName = "Foundation",
                                  serverName=NULL, asDataFrame = FALSE){
    endpoint <- sprintf("/sas/stp/%s", serverName)
